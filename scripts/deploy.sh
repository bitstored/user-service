...
docker build --tag=gcr.io/${PROJECT}/user-service .
docker push gcr.io/${PROJECT}/user-service

# Optional: Remove other old images automatically if not needed
# prev_ver=`gcloud container images list-tags gcr.io/${PROJECT}/user-service | grep -v latest | awk '{if(NR>1)print $1}'`
# if [ ! -z "$prev_ver" ]; then
#     yes | gcloud container images delete gcr.io/${PROJECT}/user-service@sha256:${prev_ver} --force-delete-tags
# fi
...
# kubectl expose deployment user-service --type=LoadBalancer --port 4001 --target-port 4001
